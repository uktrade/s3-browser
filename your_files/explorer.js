...
angular.module("aws-js-s3-explorer").factory("s3", (Config) => {
  ...
  return new AWS.S3({ endpoint: ep, s3ForcePathStyle: true });
});
...
angular
  ...
  .controller("ViewController", (Config, s3, $rootScope, $scope) => {
      ...
      try {
        ...
        $scope.$apply(function () {
          ...
          $scope.objects = response.Contents.filter((object) => {
            ...
            object.isCsv =
              object.Key.substr(object.Key.length - 3, object.Key.length) ===
              "csv";
            return object;
          });
          ...
          $scope.inBigdata = startsWith(
            currentPrefix,
            originalPrefix + Config.bigdataPrefix
          );
        });
      } catch (err) {
        $scope.$apply(() => {
...
angular
  .module("aws-js-s3-explorer")
  .controller("AddFolderController", (s3, $scope, $rootScope) => {
    ...
    $scope.addFolder = async () => {
      const withoutLeadTrailSlash = $scope.model.newFolder
        .replace(/^\/+/g, "")
        .replace(/\/+$/g, "");
...
angular
  .module("aws-js-s3-explorer")
  .controller("UploadController", (s3, $scope, $rootScope) => {
...
angular.module("aws-js-s3-explorer").controller("ErrorController", ($scope) => {
  ...
  $scope.$on("error", (e, args) => {
    ...
    const errors = objEntries(err || {}).map(([key, value]) => ({
      key,
      value,
    }));
...
angular
  .module("aws-js-s3-explorer")
  .controller("TrashController", (s3, $scope, $rootScope) => {
    ...
    $scope.deleteFiles = async () => {
      ...
      model.trashing = true;
      const [scheduleDelete, flushDelete] = Deleter(s3, model.bucket);
...
angular.module("aws-js-s3-explorer").directive("modal", () => {
  ...
  var cssEnabled =
    window.getComputedStyle(backdrop).getPropertyValue("visibility") ==
    "hidden";
